...
void cr3_callback(vmi_instance_t vmi, vmi_event_t *event) {
    ...
    while(loop) {
        ...
        access_context_t ctx = {
            // the type of address we want to access
            .as = VMI_AS_VIRTUAL,
            .addr = page->vaddr,

            // if virtual, how do we want to translate it
            .translate.type = VMI_PROCESS_DTB,
            .translate.dtb = event->reg_event.value,
        };

        uint64_t test;
        if(VMI_SUCCESS == vmi_read_64(vmi, &ctx, &test)) {
            printf("Page in virtual address space accessible: 0x"PRIx64".\t"
                "Size: 0x"PRIu64"\n",
