...
int main (int argc, char **argv)
{
    ...
    if (VMI_FAILURE == vmi_events_listen(vmi, 0)) {
        IGNORE_RETURN(fprintf(stderr, "Failed to listen on VM events\n"));
        goto error_exit;
    }
    ...
    for (vcpu=0; vcpu < num_vcpus; vcpu++) {
        if (VMI_FAILURE == vmi_toggle_single_step_vcpu(vmi, &single_event, vcpu, false)) {
            IGNORE_RETURN(fprintf(stderr, "Failed to stop singlestepping on VCPU %d\n", vcpu));
            goto error_exit;
        }
    }
    ...
    for (vcpu=0; vcpu < num_vcpus; vcpu++) {
        if (VMI_FAILURE == vmi_toggle_single_step_vcpu(vmi, &single_event, vcpu, true)) {
            IGNORE_RETURN(fprintf(stderr, "Failed to enable singlestep on VCPU %d\n", vcpu));
            goto error_exit;
        }
    }
    if (VMI_FAILURE == vmi_resume_vm(vmi)) {
