 ...
 */
static status_t get_vcpu_page_mode_x86(vmi_instance_t vmi, unsigned long vcpu,
                                       page_mode_t *out_pm) {
  ...
  if (!VMI_GET_BIT(cr0, 31)) {
    dbprint(
        VMI_DEBUG_PTLOOKUP,
        "Paging disabled for this VM, only physical addresses supported.\n");
  ...
  if (!driver_is_pv(vmi) &&
      cr3 >= vmi->max_physical_address) { // sanity check on CR3
    dbprint(VMI_DEBUG_PTLOOKUP,
            "** Note cr3 value [0x%" PRIx64
            "] exceeds max_physical_address [0x%" PRIx64 "]\n",
...
static status_t get_vcpu_page_mode_arm(vmi_instance_t vmi, unsigned long vcpu,
                                       page_mode_t *out_pm) {
  // Note: this will need to be a more comprehensive check when we start
  // supporting AArch64
  status_t ret = VMI_FAILURE;
  page_mode_t pm = VMI_PM_UNKNOWN;
  ...
  if (VMI_SUCCESS == driver_get_vcpureg(vmi, &cpsr, CPSR, vcpu)) {
    if (cpsr & PSR_MODE_BIT) {
      ...
      pm = VMI_PM_AARCH64;
      dbprint(VMI_DEBUG_PTLOOKUP,
              "Found ARM64 pagemode. TTBR0 VA width: %" PRId64
              " Page size: %u TTBR1 VA width:%" PRId64 " Page size: %u\n",
              64 - vmi->arm64.t0sz, vmi->arm64.tg0, 64 - vmi->arm64.t1sz,
              vmi->arm64.tg1);
    }
 ...
 */
status_t get_vcpu_page_mode(vmi_instance_t vmi, unsigned long vcpu,
                            page_mode_t *out_pm) {
