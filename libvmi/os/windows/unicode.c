    ...
    access_context_t *ctx)
{
    ...
    if (VMI_PM_IA32E == vmi_get_page_mode(vmi)) {   // 64 bit guest
        ...
        buffer_va = (ctx->addr & 0xFFFFFFFF00000000) + (us64.pBuffer >> 32);
    ...
    access_context_t buffer_ctx = *ctx;
    buffer_ctx.addr = buffer_va;

    read = vmi_read(vmi, &buffer_ctx, us->contents, us->length);
    if (read != us->length) {
        dbprint
