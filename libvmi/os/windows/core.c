 ...
 */
status_t get_kpgd_method2 (vmi_instance_t vmi, addr_t *sysproc)
{
    ...
    vmi->kpgd += vmi->page_offset; /* store vaddr */ //TODO fix for 64-bit
 ...
 * eprocess struct.
 */
status_t get_kpgd_method1 (vmi_instance_t vmi, addr_t *sysproc)
{
...
static status_t get_kpgd_method0 (vmi_instance_t vmi, addr_t *sysproc)
{
...
//    fprintf(stderr, "get_ntoskrnl_base returned 0x%.16x\n", tmp);
    ...
    */
    get_kpgd_method0(vmi, &sysproc);
    get_kpgd_method1(vmi, &sysproc);
    get_kpgd_method2(vmi, &sysproc);
    ...
    //
    char *buf = safe_malloc(200);
    vmi_read_va(vmi, vmi->init_task, 0, buf, 200);
    vmi_print_hex(buf, 200);
    free(buf);
    //
