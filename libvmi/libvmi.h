...
#define VMI_INIT_V2SYM                                                         \
  (1u << 5) /**< initialize address to kernel symbol translation for json      \
               profiles */
...
#define VMI_TM_NONE                                                            \
  0 /**< No translation is required, address is physical address */
#define VMI_TM_PROCESS_DTB                                                     \
  1 /**< Translate addr via specified pagetable (aka. directory table base).   \
     */
#define VMI_TM_PROCESS_PT 1 /**< Translate addr via specified pagetable. */
#define VMI_TM_PROCESS_PID                                                     \
  2 /**< Translate addr by finding process first to use its DTB. */
#define VMI_TM_KERNEL_SYMBOL                                                   \
  3 /**< Find virtual address of kernel symbol and translate it via kernel     \
       DTB. */
 ...
 * @param[out] error Optional. If not NULL and the function returns
 * VMI_OS_UNKNOWN, this will specify the stage at which initialization failed.
 * @return VMI_OS_UNKNOWN when the configuration didn't work for the VM,
 * otherwise the OS type that has been initialized (ie. VMI_OS_WINDOWS or
 *         VMI_OS_LINUX).
 */
os_t vmi_init_os(vmi_instance_t vmi, vmi_config_t config_mode, void *config,
                 vmi_init_error_t *error) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_translate_kv2p(vmi_instance_t vmi, addr_t vaddr,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_translate_uv2p(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                            addr_t *paddr) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_translate_ksym2v(vmi_instance_t vmi, const char *symbol,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_translate_sym2v(vmi_instance_t vmi, const access_context_t *ctx,
                             const char *symbol, addr_t *vaddr) NOEXCEPT;
 ...
 */
const char *vmi_translate_v2sym(vmi_instance_t vmi, const access_context_t *ctx,
                                addr_t rva) NOEXCEPT;
 ...
 * @return Symbol, or NULL on error
 */
const char *vmi_translate_v2ksym(vmi_instance_t vmi,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_pid_to_dtb(vmi_instance_t vmi, vmi_pid_t pid,
                        addr_t *dtb) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_dtb_to_pid(vmi_instance_t vmi, addr_t dtb,
...
status_t vmi_pagetable_lookup(vmi_instance_t vmi, addr_t pt, addr_t vaddr,
                              addr_t *paddr) NOEXCEPT;
 ...
 */
status_t vmi_pagetable_lookup_extended(vmi_instance_t vmi, addr_t pt,
                                       addr_t vaddr,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_nested_pagetable_lookup(vmi_instance_t vmi, addr_t npt,
                                     page_mode_t npm, addr_t pt, page_mode_t pm,
                                     addr_t vaddr, addr_t *paddr,
 ...
 */
status_t vmi_read(vmi_instance_t vmi, const access_context_t *ctx, size_t count,
                  void *buf, size_t *bytes_read) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_8(vmi_instance_t vmi, const access_context_t *ctx,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_16(vmi_instance_t vmi, const access_context_t *ctx,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_32(vmi_instance_t vmi, const access_context_t *ctx,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_64(vmi_instance_t vmi, const access_context_t *ctx,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_addr(vmi_instance_t vmi, const access_context_t *ctx,
 ...
 * @return String read from memory or NULL on error
 */
uint16_t *vmi_read_wstr(vmi_instance_t vmi,
 ...
 */
unicode_string_t *vmi_read_unicode_str(vmi_instance_t vmi,
                                       const access_context_t *ctx) NOEXCEPT;
 ...
 * @return VMI_SUCCESS if read is complete, VMI_FAILURE otherwise
 */
status_t vmi_read_ksym(vmi_instance_t vmi, const char *sym, size_t count,
                       void *buf, size_t *bytes_read) NOEXCEPT;
...
status_t vmi_read_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                     size_t count, void *buf, size_t *bytes_read) NOEXCEPT;

/**
 * Maps num_pages of the guest's virtual memory into host, starting at the
 * provided vaddr. Each page will have it's own pointer in access_ptrs output
 * array. Be aware that not all virtual pages may be allocated and in such case,
 * the corresponding array item will be set to NULL. Remember to call munmap()
 * on each array item afterwards.
 *
 * @param[in] vmi LibVMI instance
 * @param[in] ctx Access context
 * @param[in] num_pages Number of guest pages to be mapped (starting from
 * ctx.addr)
 * @param[in] prot Memory protection flags
 * @param[out] access_ptrs Output array of size [num_pages] containing pointers
 * to the respective guest's pages
 */
status_t vmi_mmap_guest(vmi_instance_t vmi, const access_context_t *ctx,
                        size_t num_pages, int prot,
                        void **access_ptrs) NOEXCEPT;

/**
 * Maps num_pages of the guest's virtual memory into the host, starting at the
 * provided virtual address. Writes the result into an array consisting of
 * mapped_region structs. A mapped_region describes a chunk of contiguous
 * virtual memory. Each mapped_region struct holds the base pointer as well as
 * the starting GVA and the size of such a mapped region. The caller is
 * responsible for freeing the array via vmi_free_mapped_regions() which in turn
 * will also call munmap() on every single mapped region.
 *
 * This function performs a lot better than vmi_mmap_guest() if a large,
 * sparsely mapped memory region is requested. Results between this function and
 * vmi_mmap_guest() may vary due to different v2p translation mechanisms being
 * used.
 *
 * @param[in] vmi LibVMI instance
 * @param[in] ctx Access context
 ...
 * ctx.addr)
 * @param[in] prot Memory protection flags
 * @param[out] mapped_regions A struct containing the location and length of a
 * dynamically allocated array of mapped_region structs.
 * @return VMI_SUCCESS if mapping succeeds, VMI_FAILURE otherwise
 */
status_t vmi_mmap_guest_2(vmi_instance_t vmi, const access_context_t *ctx,
                          size_t num_pages, int prot,
                          mapped_regions_t *mapped_regions) NOEXCEPT;

/**
 * Frees the content of a mapped_regions struct. Will also unmap each region in
 * the process.
 *
 * @param[in] vmi LibVMI instance
 * @param[in] mapped_regions A mapped_regions struct. The struct itself will not
 * be freed.
 */
void vmi_free_mapped_regions(vmi_instance_t vmi,
                             const mapped_regions_t *mapped_regions) NOEXCEPT;

/**
 * Maps num_pages of the guest's physical memory into host, starting at the
 * provided paddr. Each page will have it's own pointer in access_ptrs output
 * array. Remember to call munmap() on each array item afterwards.
 *
 * @param[in] vmi LibVMI instance
 ...
 * to the respective guest's pages
 */
status_t vmi_mmap_guest_pa(vmi_instance_t vmi, addr_t paddr, size_t num_pages,
                           int prot, void **access_ptrs) NOEXCEPT;
 ...
 * @return VMI_SUCCESS if read is complete, VMI_FAILURE otherwise
 */
status_t vmi_read_pa(vmi_instance_t vmi, addr_t paddr, size_t count, void *buf,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_8_ksym(vmi_instance_t vmi, const char *sym,
                         uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_16_ksym(vmi_instance_t vmi, const char *sym,
                          uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_32_ksym(vmi_instance_t vmi, const char *sym,
                          uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_64_ksym(vmi_instance_t vmi, const char *sym,
                          uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_addr_ksym(vmi_instance_t vmi, const char *sym,
                            addr_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_8_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                       uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_16_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                        uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_32_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                        uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_64_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                        uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_addr_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                          addr_t *value) NOEXCEPT;
 ...
 * @return String read from memory or NULL on error
 */
uint16_t *vmi_read_wstr_va(vmi_instance_t vmi, addr_t vaddr,
                           vmi_pid_t pid) NOEXCEPT;
 ...
 * string to the "UTF-8" encoding and output with printf("%s") NOEXCEPT; (2)
 * convert a string to the "WCHAR_T" encoding and output with printf("%ls").
 *
 * @param[in] in  unicode_string_t to be converted; encoding field must be set
 * @param[in] out output unicode_string_t, allocated by caller (this function
 * allocates the contents field)
 * @param[in] outencoding output encoding, must be compatible with the iconv
 * function
 * @return status code
 */
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_8_pa(vmi_instance_t vmi, addr_t paddr,
                       uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_16_pa(vmi_instance_t vmi, addr_t paddr,
                        uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_32_pa(vmi_instance_t vmi, addr_t paddr,
                        uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_64_pa(vmi_instance_t vmi, addr_t paddr,
                        uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_addr_pa(vmi_instance_t vmi, addr_t paddr,
                          addr_t *value) NOEXCEPT;
 ...
 * @return String read from memory or NULL on error
 */
uint16_t *vmi_read_wstr_pa(vmi_instance_t vmi, addr_t paddr) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write(vmi_instance_t vmi, const access_context_t *ctx,
                   size_t count, void *buf, size_t *bytes_written) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_ksym(vmi_instance_t vmi, const char *sym, size_t count,
                        void *buf, size_t *bytes_written) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                      size_t count, void *buf, size_t *bytes_written) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_pa(vmi_instance_t vmi, addr_t paddr, size_t count, void *buf,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_8(vmi_instance_t vmi, const access_context_t *ctx,
                     uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_16(vmi_instance_t vmi, const access_context_t *ctx,
                      uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_32(vmi_instance_t vmi, const access_context_t *ctx,
                      uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_64(vmi_instance_t vmi, const access_context_t *ctx,
                      uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_addr(vmi_instance_t vmi, const access_context_t *ctx,
                        addr_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_8_ksym(vmi_instance_t vmi, const char *sym,
                          uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_16_ksym(vmi_instance_t vmi, const char *sym,
                           uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_32_ksym(vmi_instance_t vmi, const char *sym,
                           uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_64_ksym(vmi_instance_t vmi, const char *sym,
                           uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_addr_ksym(vmi_instance_t vmi, const char *sym,
                             addr_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_8_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                        uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_16_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                         uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_32_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                         uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_64_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                         uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_addr_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                           addr_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_8_pa(vmi_instance_t vmi, addr_t paddr,
                        uint8_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_16_pa(vmi_instance_t vmi, addr_t paddr,
                         uint16_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_32_pa(vmi_instance_t vmi, addr_t paddr,
                         uint32_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_64_pa(vmi_instance_t vmi, addr_t paddr,
                         uint64_t *value) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_write_addr_pa(vmi_instance_t vmi, addr_t paddr,
                           addr_t *value) NOEXCEPT;
 ...
 */
void vmi_print_hex_ksym(vmi_instance_t vmi, const char *sym,
 ...
 * @param[in] length The length (in bytes) of data to print
 */
void vmi_print_hex_va(vmi_instance_t vmi, addr_t vaddr, vmi_pid_t pid,
                      size_t length) NOEXCEPT;
 ...
 */
status_t vmi_get_access_mode(vmi_instance_t vmi, const void *domain,
                             uint64_t init_flags, vmi_init_data_t *init_data,
 ...
 */
bool vmi_get_windows_build_info(vmi_instance_t vmi,
                                win_build_info_t *info) NOEXCEPT;
...
/**
 * Get string represenatation of the version of Windows that LibVMI is currently
 * accessing.
 *
 * @param[in] vmi LibVMI instance
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_offset(vmi_instance_t vmi, const char *offset_name,
                        addr_t *offset) NOEXCEPT;

/**
 * Get the memory offset associated with the given symbol and subsymbol in the
 * struct
 * @param[in] vmi LibVMI instance
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_kernel_struct_offset(vmi_instance_t vmi,
 ...
 *
 * @param[in] vmi LibVMI instance
 * @param[in] vcpu The index of the VCPU to access, use 0 for single VCPU
 * systems
 * @param[out] xsave_info Returned value of the xsave_info
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_xsave_info(vmi_instance_t vmi, unsigned long vcpu,
...
vmi_get_last_pagetable_lookup_fault(vmi_instance_t vmi) NOEXCEPT;
...
/**
 * Retrieves the next gfn that would be available for allocation of physical
 * memory. In some cases, the maximum physical address API is not sufficient for
 * obtaining unoccupied memory due to implementation details of certain
 * introspection target platforms (e.g. KVM/QEMU). This API guarantees that the
 * return value represents unoccupied physical memory.
 *
 * @param[in] vmi LibVMI instance @return physical memory size
 * @return next available gfn
 */
addr_t vmi_get_next_available_gfn(vmi_instance_t vmi) NOEXCEPT;
...
/**
 * Injects a page fault trap. It is assumed that the guest is in user-mode and
 * in the proper address space for the request to work.
 *
 * @param[in] vmi LibVMI instance
 * @param[in] vcpu The index of the VCPU to access, use 0 for single VCPU
 * systems
 * @param[in] virtual_address The cr2 address for requested page fault
 * @param[in] error_code The error code for the requested page fault. ~0u means
 * 'ignore'
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_request_page_fault(vmi_instance_t vmi, unsigned long vcpu,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_tsc_info(vmi_instance_t vmi, uint32_t *tsc_mode,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_vcpumtrr(vmi_instance_t vmi, mtrr_regs_t *hwMtrr,
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_vcpureg(vmi_instance_t vmi, uint64_t *value, reg_t reg,
                         unsigned long vcpu) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_get_vcpuregs(vmi_instance_t vmi, registers_t *regs,
                          unsigned long vcpu) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_set_vcpureg(vmi_instance_t vmi, uint64_t value, reg_t reg,
                         unsigned long vcpu) NOEXCEPT;

/**
 * Sets the vCPU registers to the ones passed in the struct. It is important to
 * have a valid value in all registers when calling this function, so the user
 * likely wants to call vmi_get_vcpuregs before calling this function. When
 * LibVMI is accessing a raw memory file or KVM, this function will fail.
 *
 * @param[in] vmi LibVMI instance
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_set_vcpuregs(vmi_instance_t vmi, registers_t *regs,
                          unsigned long vcpu) NOEXCEPT;
 ...
 */
status_t vmi_alloc_gfn(vmi_instance_t vmi, uint64_t gfn) NOEXCEPT;
 ...
 */
status_t vmi_free_gfn(vmi_instance_t vmi, uint64_t gfn) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
void vmi_v2pcache_nested_flush(vmi_instance_t vmi, addr_t pt,
                               addr_t npt) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
void vmi_v2pcache_add(vmi_instance_t vmi, addr_t va, addr_t pt,
                      addr_t pa) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
void vmi_v2pcache_nested_add(vmi_instance_t vmi, addr_t va, addr_t pt,
                             addr_t npt, addr_t pa) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
void vmi_symcache_add(vmi_instance_t vmi, addr_t base_addr, vmi_pid_t pid,
                      char *sym, addr_t va) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
void vmi_rvacache_add(vmi_instance_t vmi, addr_t base_addr, vmi_pid_t pid,
                      addr_t rva, char *sym) NOEXCEPT;
...
/**
 * Only for Xen-based VMs. Reads count bytes from disk (not cdrom) identified by
 * device_id, which can be retrieved with vmi_get_disks()
 *
 * @param[in] vmi LibVMI instance
 * @param[in] device_id vbd device id
 * @param[in] offset Starting disk offset to read from
 * @param[in] count Number of bytes to read
 * @param[out] buffer Output buffer for storing read data. Must be allocated by
 * caller
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_read_disk(vmi_instance_t vmi, const char *device_id,
                       uint64_t offset, uint64_t count, void *buffer) NOEXCEPT;

/**
 * Only for Xen-based VMs. Retrieves list of available hard disks on VM. Caller
 * must free() returned buffer after usage.
 *
 * @param[in] vmi LibVMI instance
 * @param[out] num Pointer to number of returned items
 * @return Array pointers, consisting of num elements or NULL in case of error.
 * Each element represents a string disk identifier
 */
char **vmi_get_disks(vmi_instance_t vmi, unsigned int *num) NOEXCEPT;
 ...
 * @return VMI_SUCCESS or VMI_FAILURE
 */
status_t vmi_disk_is_bootable(vmi_instance_t vmi, const char *device_id,
                              bool *bootable) NOEXCEPT;

/**
 * Get the execution status of the domain associated with the provided
 * vmi_instance
 *
 * @param[in] vmi LibVMI instance
 * @param[out] domain_status domain_status_t containing the status flags for the
 * given domain
 *
 * @return status_t result of retrieving the domain's status.
 */
status_t vmi_get_domain_status(vmi_instance_t vmi,
