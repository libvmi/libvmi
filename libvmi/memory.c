    ...
    uint32_t pdpe)
{
    uint32_t value;
    uint32_t pgd_entry =
        pdba_base_nopae(pdpe) + pgd_index(instance, vaddr);
    ...
    uint64_t pdpe)
{
    uint64_t value;
    uint32_t pgd_entry =
        pdba_base_pae(pdpe) + pgd_index(instance, vaddr);
    dbprint("--PTLookup: pgd_entry = 0x%.8x\n", pgd_entry);
    ...
    uint64_t pdpte)
{
    uint64_t value = 0;
    addr_t pde_address =
        get_bits_51to12(pdpte) | get_pd_index_ia32e(vaddr);
    ...
    uint32_t pgd)
{
    uint32_t value;
    uint32_t pte_entry =
        ptba_base_nopae(pgd) + pte_index(instance, vaddr);
    ...
    uint64_t pgd)
{
    uint64_t value;
    uint32_t pte_entry =
        ptba_base_pae(pgd) + pte_index(instance, vaddr);
    dbprint("--PTLookup: pte_entry = 0x%.8x\n", pte_entry);
    ...
    uint64_t pde)
{
    uint64_t value = 0;
    addr_t pte_address =
        get_bits_51to12(pde) | get_pt_index_ia32e(vaddr);
...
get_gigpage_ia32e(
    ...
    uint64_t pdpte)
{
    return (pdpte & 0x000FFFFFC0000000ULL) | (vaddr &
...
get_2megpage_ia32e(
    ...
    uint64_t pde)
{
    return (pde & 0x000FFFFFFFE00000ULL) | (vaddr &
    ...
    if (!dtb) {
        ...
        if (!rtnval) {
            if (VMI_SUCCESS == pid_cache_del(vmi, pid)) {
                return vmi_translate_uv2p_nocache(vmi, virt_address,
                                                  pid);
            }
        }
        return rtnval;
    ...
    char *symbol)
{
    ...
    if (VMI_FAILURE == sym_cache_get(vmi, symbol, &ret)) {
        if (VMI_OS_LINUX == vmi->os_type) {
            ...
            if (VMI_FAILURE ==
                windows_symbol_to_address(vmi, symbol, &ret)) {
                ret = 0;
            }
        }
